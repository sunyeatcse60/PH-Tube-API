...
//   console.log(videos);
  videos.forEach((video) => {
    ...
    card.innerHTML = `
        ...
        ${
            video.others.posted_date?.length == 0 ? "" : `<span class="absolute right-2 bottom-2 bg-black text-white rounded text-small p-1">${getTime(video.others.posted_date)}</span>`
        }
...
const displayCategories = (categories) => {
  ...
  categories.forEach((items) => {
    ...
    <button onclick="loadCategoriesVideos(${items.category_id})" class ="btn">
    ${items.category}
    </button>
    `; 
